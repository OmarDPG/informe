        ...
        <header class="flex items-center h-10 px-4 bg-gray-900 text-white rounded-t-lg">
            <div>
                <div class="flex pl-3 w-full">
                    ...
                    <span class="pl-2 pt-0.5 font-normal"><?php echo $current ?? 'Evidencia / Glosas de Gobierno'; ?></span>
                </div>
            </div>
                    ...
                    <table class="table table-bordered sizeFontTable">
                        <tbody>
                            <tr>
                                ...
                                <td class="pt-4" tabindex="43"><a href="<?php echo base_url() . '/administrador/glosas'; ?>" class="pl-4 mb-1 flex place-items-center text-emerald-600/100">
            ...
            <div class="w-full sm:w-4/5 mx-auto bg-white shadow-lg overflow-y-auto" style="max-height: calc(100vh - 200px);">
                <section class="container mx-auto px-6 py-6">
                    ...
                    <div class="mb-8 text-center">
                        ...
                        <p class="text-gray-600">Pase el cursor sobre cada unidad para ver las glosas registradas</p>
                        <div class="mt-4 flex justify-center gap-4 text-sm">
                            <div class="flex items-center">
                                <span class="w-3 h-3 bg-green-500 rounded-full mr-2"></span>
                                <span>Con glosas</span>
                            </div>
                            <div class="flex items-center">
                   ...
                   <!-- Filtros -->
                    <div class="mb-6 flex flex-wrap gap-4 justify-between items-center bg-gray-50 p-4 rounded-lg">
                        <div class="flex gap-4 flex-wrap">
                            <div class="flex items-center gap-2">
                                ...
                                <select id="filterYear" class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg p-2">
                        ...
                        <div>
                            <input type="text" id="searchUnidad" placeholder="Buscar unidad..." class="bg-white border border-gray-300 text-gray-900 text-sm rounded-lg p-2">
                        </div>
                    </div>

                    <!-- Grid de Unidades -->
                    <div id="unidadesGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"></div>
                    ...
                    <div id="loadingState" class="flex justify-center items-center py-20">
                        ...
                        <span class="ml-3 text-gray-600">Cargando glosas...</span>
                    </div>

                    <!-- Empty State -->
                    <div id="emptyState" class="hidden text-center py-20">
                        <i class="fa-solid fa-folder-open text-6xl text-gray-300 mb-4"></i>
                        <p class="text-gray-500 text-lg">No se encontraron glosas</p>
                    </div>
                </section>
            </div>
        </div>
...
<!-- Modal para ver detalles de la glosa -->
<div id="glosaModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
...
<script>
    ...
    async function loadUnidadesData() {
        try {
            ...
            const response = await fetch('<?php echo base_url(); ?>/administrador/getUnidadesConGlosas');
            const data = await response.json();

            unidadesData = data.unidades || [];
            glosasData = data.glosas || [];
    ...
    function renderUnidades(unidades) {
        ...
        grid.innerHTML = unidades.map(unidad => {
            const glosas = glosasData.filter(glo => glo.id_unidad === unidad.id_unidad);
            const hasGlosas = glosas.length > 0;
            const statusColor = hasGlosas ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200';
            const statusIcon = hasGlosas ? 'fa-check-circle text-green-500' : 'fa-circle text-gray-400';
            ...
            <div class="unidad-card ${statusColor} rounded-lg border-2 p-5 hover:shadow-xl">
                ...
                <div class="flex items-center justify-between py-3 border-t border-gray-200 mt-3">
                    <div class="flex items-center">
                        ...
                        <span class="text-sm font-semibold text-gray-700">
                            ${glosas.length} ${glosas.length === 1 ? 'Glosa' : 'Glosas'}
                        </span>
                    </div>
                    ${hasGlosas ? `
                ...
                ${hasGlosas ? `
                    <div class="glosas-list mt-4 pt-4 border-t border-gray-200" style="overflow-y: auto;">
                        <h4 class="text-sm font-semibold text-gray-700 mb-3">
                            <i class="fa-solid fa-list-ul mr-2"></i>Glosas Registradas:
                        </h4>
                                        ...
                                        <div class="flex items-center justify-between">
                                            <div class="flex-1">
                                                <div class="flex items-center gap-2 mb-1">
                                                    ...
                                                    <span class="badge-status badge-${glosa.estado}">
                                                        ${getEstadoLabel(glosa.estado)}
                                                    </span>
                                                </div>
                                                ${glosa.fecha ? `
                                                    <p class="text-xs text-gray-500">
                                                        <i class="fa-solid fa-calendar mr-1"></i>
                                                        ${formatDate(glosa.fecha)}
                                                    </p>
                                                ` : ''}
                                            </div>
    ...
    function verDetalleUnidad(idUnidad) {
        ...
        <div class="mb-6">
            <h5 class="text-lg font-semibold text-gray-800 mb-4">Resumen de Glosas</h5>
            <div class="grid grid-cols-3 gap-4">
                <div class="bg-green-50 rounded-lg p-4 text-center">
                    <p class="text-3xl font-bold text-green-600">${glosas.filter(g => g.estado === 'aprobado').length}</p>
        ...
        <div>
            <h5 class="text-lg font-semibold text-gray-800 mb-4">Historial de Glosas</h5>
            <div class="space-y-3">
                ${glosas.map(glosa => {
                    const match = glosa.nombre_glosa?.match(/\b(20\d{2})\b/);
                    const anio = match ? match[1] : '';
        ...
        document.getElementById('glosaModal').classList.remove('hidden');
    }
    ...
    // Cerrar modal
    function closeModal() {
        document.getElementById('glosaModal').classList.add('hidden');
    }
    ...
    document.getElementById('glosaModal')?.addEventListener('click', function(e) {
        if (e.target === this) {
            closeModal();
        }
    });
