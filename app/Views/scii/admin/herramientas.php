<div class="p-8">
        <section>
            ...
            <div>
                ...
                <a href="<?php echo base_url();?>/administrador/inicio" class="w-5 h-5 bg-red-500 rounded-md ml-auto text-white text-md font-light hover:bg-blue-gray-light hover:cursor-pointer focus:outline-none">
            ...
            <div class="flex shadow-2xl" style="min-height: 18rem; max-height: 30rem;">
                <div class="sm:w-1/4 w-2/5 overflow-auto bg-gray-100">
                    ...
                    <div class="mt-2 px-5">
                        ...
                        <a href="<?php echo base_url().'/administrador/herramientas';?>" class="pl-4 mb-1 flex place-items-center text-emerald-600/100"><i class="fa-solid fa-folder pr-1"></i>
                            Herramientas
                        </a>
                            ...
                            <div class="grid grid-cols-1 py-3">
                                ...
                                <div class="relative z-0 w-full group">
                                    <div class="flex mx-2 hover:bg-zinc-200" id="<?php echo 'button'.$i;?>">
                                        <span class="text-rose-900 self-center text-sm w-full"><?php echo 'Programa de Trabajo';?></span>
                                        
                                        <?php 
                                        $files = glob('files/herramientas/'.$i.'/*');
                                        $det=isset($files[0])?(pathinfo($files[0], PATHINFO_EXTENSION)):'file';
                                        ?>
                                        <button onclick="window.open('<?php echo base_url().'/files/herramientas/'.$i.'/'.$i.'.'.$det?>')" class="hover:bg-white bg-rose-900 hover:text-rose-900 text-white font-bold my-2 py-1 border hover:border-rose-900 w-1/5 lg:w-1/12 rounded-l-lg">
                                            <i class="fa-solid fa-eye"></i>
                                        </button>
                                        <button id="<?php echo 'button'.$i;?>" name="<?php echo 'button'.$i;?>" onclick="<?php echo 'thisFileUpload'.$i;?>();" class="hover:bg-white bg-rose-900 hover:text-rose-900 text-white w-1/5 lg:w-1/12 font-bold my-2 py-1 border hover:border-rose-900 rounded-r-lg">
                                ...
                                <?php $formid='myForm'.$i;?>
                                <form action="<?php echo base_url().'/administrador/uploadH/'.$i?>" id="<?php echo $formid;?>" class="hidden" enctype="multipart/form-data" method="post" accept-charset="utf-8">
                                    <input type="file" name="<?php echo 'userfile'.$i;?>" id="<?php echo 'userfile'.$i;?>" onchange="<?php echo 'change'.$i;?>()" class="hidden" >
                                    <input type="submit" id="<?php echo 'submit'.$i;?>" value="Go">
                                </form>
                            </div>
                        </div>
                        <div class="mb-1">
                            <h1 class="font-bold text-md">Administración de Riesgos</h1>
                            <div class="grid grid-cols-1 py-3 ">
                                <?php for($i=2;$i<=3;$i++){?>
                                    <div class="relative z-0 w-full group">
                                        <div class="flex mx-2 hover:bg-zinc-200" id="<?php echo 'button'.$i;?>">
                                            <span class="text-rose-900 self-center text-sm w-full"><?php echo ($i==2)?'Programa de Trabajo de Adm Riesgos':'Matriz y Mapa de Riesgos';?></span>
                                            <?php 
                                                $files = glob('files/herramientas/'.$i.'/*');
                                                $det=isset($files[0])?(pathinfo($files[0], PATHINFO_EXTENSION)):'file';
                                            ?>
  ...
  <nav class="fixed bottom-0 w-full z-40">
    ...
    <?php for($i=1;$i<=9;$i++){ $formid='myForm'.$i;?>
    <script>
        $("#<?php echo $formid;?>" ).on( "submit", function( event ) {
        event.preventDefault();
        $.ajax( {
            url: '<?php echo base_url().'/administrador/uploadH/'.$i?>',
            type: 'POST',
