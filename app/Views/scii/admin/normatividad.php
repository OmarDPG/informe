        ...
        <section>
            ...
            <header class="flex items-center h-10 px-4 bg-gray-900 text-white rounded-t-lg">
                ...
                <div class="flex items-center">
                    <button id="button" name="button" value="Seleccionar archivo" class="w-5 h-5 mr-1 bg-emerald-600 rounded-md ml-auto text-white text-lg font-light hover:bg-blue-gray-light hover:cursor-pointer focus:outline-none" onclick="thisFileUpload();">
                    ...
                    <form action="<?php echo base_url().'/administrador/upload'?>" id="myForm" class="hidden" enctype="multipart/form-data" method="post" accept-charset="utf-8">
                        <input type="file" name="userfile" id="userfile" onchange="change()" class="hidden" accept="application/pdf" >
            ...
            <div class="flex shadow-2xl" style="min-height: 18rem; max-height: 30rem;">
                <div class="sm:w-1/5 w-1/5 overflow-auto bg-gray-100">
                    ...
                    <div class="mt-2 px-5">
                        <a href="<?php echo base_url().'/administrador/normatividad';?>" class="pl-4 mb-1 flex place-items-center text-emerald-600/100"><i class="fa-solid fa-folder pr-1"></i>
                            Normatividad
                        </a>
                                        ...
                                        <table id="tablaDocs" class="min-w-full divide-y divide-gray-200">
                                            ...
                                            <tbody class="bg-white divide-y divide-gray-200">
                                                <?php foreach($map as $mp){?>
                                                    <tr>
                                                    <td class="px-4 py-4 text-sm text-gray-500 whitespace-nowrap"><i class="fa-solid fa-file"></i> <?php echo $mp;?></td>
                                                    <td class="px-4 py-4 text-sm text-gray-500 whitespace-nowrap"><?php echo date ("d/m/Y H:i:s", filemtime('files/normatividad/'.$mp))?></td>
                                                    <td class="px-4 py-4 text-sm text-gray-500 whitespace-nowrap"><?php echo pathinfo('files/normatividad/'.$mp, PATHINFO_EXTENSION);?></td>
                                                    <td class="px-4 py-4 text-sm whitespace-nowrap">
                                                        <div class="flex items-center gap-x-6">
                                                            ...
                                                            <a href="<?php echo base_url().'/files/normatividad/'.$mp;?>" download class="text-gray-500 transition-colors duration-200 hover:text-emerald-500 focus:outline-none">
    ...
    $("#myForm" ).on( "submit", function( event ) {
        ...
        $.ajax({
            url: '<?php echo base_url();?>/administrador/upload',
            type: 'POST',
    ...
    function fileDelete(mp){
        if(mp != null){
            event.preventDefault();
            $.ajax({
                url: '<?php echo base_url(); ?>/administrador/delNormatividad/' + mp,
                success: function(result){
                    if(result==0){
