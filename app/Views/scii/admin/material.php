    <div class="p-8 rounded-lg">
        <section>
            ...
            <div class="flex shadow-2xl" style="min-height: 18rem; max-height: 37rem;">
                <div class="sm:w-1/4 w-2/5 overflow-auto bg-gray-100">
                    ...
                    <div class="mt-2 px-5">
                        ...
                        <a href="<?php echo base_url().'/administrador/material';?>" class="pl-4 mb-1 flex place-items-center text-emerald-600/100"><i class="fa-solid fa-folder pr-1"></i>
                            Material
                        </a>
                        <?php for($i=2023;$i<=date('Y');$i++){?>
                        <div class="mt-2 px-7 hover:bg-zinc-200">
                            <a href="<?php echo base_url().'/administrador/material/'.$i;?>" class="pl-4 mb-1 flex place-items-center"><i class="fa-regular fa-folder pr-1"></i>
                                <?php echo $i;?>
                            </a>
                        </div>
                        <?php }?>
                ...
                <div class="sm:w-3/4 w-3/5 overflow-auto bg-gray-50">
                    <div class="grid grid-cols-1 p-3">
                        <div class="mb-1">
                            <!-- Image loader -->
                            <div id='loader' class="fixed top-0 left-0 w-screen h-screen flex items-center justify-center" style='display: none;'>
                                <i class="fa-solid text-rose-900 fa-spinner fa-spin-pulse text-9xl"></i>
                            </div>
                            <!-- Image loader -->
                            <h1 class="font-bold mb-3">Multimedia (Imágenes, presentaciones y video)</h1>
                            <h1 class="font-bold text-md">Control interno</h1>
                            <div class="grid grid-cols-1 py-3">
                                <?php for($i=0;$i<=3;$i++){?>
                                    <div class="relative z-0 w-full group">
                                        <div class="flex mx-2 hover:bg-zinc-200" id="<?php echo 'button'.$i;?>">
                                            <span class="text-rose-900 self-center text-sm w-full"><?php echo $i==0?'Video':($i==1?'¿Qué es el Control interno?':($i==2?'Normas Generales, Principios y Elem.':'Sistema de Evaluación'));?></span>
                                            <?php 
                                                $files = glob('files/materiales/'.$i.'/*');
                                                $det=isset($files[0])?(pathinfo($files[0], PATHINFO_EXTENSION)):'file';
                                            ?>
                                            <button id="<?php echo 'watch'.$i?>" <?php echo isset($files[0])?'onclick='."window.open('".base_url()."/files/materiales/".$i."/".$i.".".$det."')".'':'';?> class="hover:bg-white bg-rose-900 hover:text-rose-900 text-white font-bold my-2 py-1 border hover:border-rose-900 w-1/5 lg:w-1/12 rounded-l-lg">
                                                <i class="fa-solid fa-eye"></i>
                                            </button>
                                            <button id="<?php echo 'button'.$i;?>" name="<?php echo 'button'.$i;?>" value="Seleccionar archivo" class="hover:bg-white bg-rose-900 hover:text-rose-900 text-white w-1/5 lg:w-1/12 font-bold my-2 py-1 border hover:border-rose-900 rounded-r-lg" onclick="<?php echo 'thisFileUpload'.$i;?>();">
                                                <i class="fa-solid fa-upload"></i>
                                            </button>
                                        </div>
                                    </div>
                                    <script>
                                        function <?php echo 'thisFileUpload'.$i;?>() {
                                            document.getElementById("<?php echo 'userfile'.$i;?>").click();
                                        };
                                        function <?php echo 'change'.$i;?>(){
                                            document.getElementById("<?php echo 'submit'.$i;?>").click();
                                        }
                                    </script>
                                    <?php $formid='myForm'.$i;?>
                                    <?= form_open_multipart('administrador/uploadM/'.$i,array('id' => $formid, 'class'=>'hidden')) ?>
                                        <input type="file" name="<?php echo 'userfile'.$i;?>" id="<?php echo 'userfile'.$i;?>" onchange="<?php echo 'change'.$i;?>()" class="hidden" accept="<?php echo $i==0?'video/mp4':'application/*';?>" >
                                        <input type="submit" id="<?php echo 'submit'.$i;?>" value="Go">
                                    <?=form_close();?>
                                <?php }?>
                            </div>
                            <h1 class="font-bold text-md">COCODI</h1>
                            <div class="grid grid-cols-1 py-3">
                                <?php for($i=4;$i<=4;$i++){?>
                                    <div class="relative z-0 w-full group">
                                        <div class="flex mx-2 hover:bg-zinc-200" id="<?php echo 'button'.$i;?>">
                                            <span class="text-rose-900 self-center text-sm w-full"><?php echo $i==4?'Organigrama':'NUEVO ELEMENTO';?></span>
                                            <?php 
                                                $files = glob('files/materiales/'.$i.'/*');
                                                $det=isset($files[0])?(pathinfo($files[0], PATHINFO_EXTENSION)):'file';
                                            ?>
                                    ...
                                    <?php $formid='myForm'.$i;?>
                                    <?= form_open_multipart('administrador/uploadM/'.$i,array('id' => $formid, 'class'=>'hidden')) ?>
                                        <input type="file" name="<?php echo 'userfile'.$i;?>" id="<?php echo 'userfile'.$i;?>" onchange="<?php echo 'change'.$i;?>()" class="hidden" accept="image/png, image/gif, image/jpeg" >
                                        <input type="submit" id="<?php echo 'submit'.$i;?>" value="Go">
                                    <?=form_close();?>
                                <?php }?>
                            </div>
    ...
    <?php for($i=0;$i<=4;$i++){ $formid='myForm'.$i;?>
    <script>
        $("#<?php echo $formid;?>" ).on( "submit", function( event ) {
        $("#loader").show();
        event.preventDefault();
        $.ajax( {
            url: '<?php echo base_url().'/administrador/uploadM/'.$i?>',
            method: 'POST',
            ...
            success: function(result){
                ...
                if(result==0){
                    ...
                    if(resultado.determinante != ''){
                        $("#<?php echo 'watch'.$i;?>" ).attr('onclick','window.open("<?php echo base_url()."/files/materiales/".$i."/".$i.'.';?>'+resultado.determinante+'")')
                    }else{
